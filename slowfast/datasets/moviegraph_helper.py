...
def load_image_lists(cfg, is_train):
    ...
    """
    list_filenames = [
        os.path.join(cfg.MOVIEGRAPH.FRAME_LIST_DIR, filename)
        for filename in (
            cfg.MOVIEGRAPH.TRAIN_LISTS if is_train else cfg.MOVIEGRAPH.TEST_LISTS
        )
    ]
        ...
        with PathManager.open(list_filename, "r") as f:
            ...
            for line in f:
                ...
                image_paths[data_key].append(
                    os.path.join(cfg.MOVIEGRAPH.FRAME_DIR, row[1])
                )
...
def load_boxes_and_labels(cfg, mode):
    ...
    Returns:
        all_boxes (dict): a dict which maps from `video_name` and
            ...
            labels for the box.
    """
    gt_lists = cfg.MOVIEGRAPH.TRAIN_GT_BOX_LISTS if mode == "train" else []
    pred_lists = (
        cfg.MOVIEGRAPH.TRAIN_PREDICT_BOX_LISTS
        if mode == "train"
        else cfg.MOVIEGRAPH.TEST_PREDICT_BOX_LISTS
    )
    ann_filenames = [
        os.path.join(cfg.MOVIEGRAPH.ANNOTATION_DIR, filename)
        for filename in gt_lists + pred_lists
    ]
    ann_is_gt_box = [True] * len(gt_lists) + [False] * len(pred_lists)

    detect_thresh = cfg.MOVIEGRAPH.DETECTION_SCORE_THRESH
    # Only select frame_sec % 4 = 0 samples for validation if not
    # set FULL_TEST_ON_VAL.
    boxes_sample_rate = (
        4 if mode == "val" and not cfg.MOVIEGRAPH.FULL_TEST_ON_VAL else 1
    )
...
def get_keyframe_data(boxes_and_labels):
    ...
    for video_idx in range(len(boxes_and_labels)):
        ...
        for sec in boxes_and_labels[video_idx].keys():
            if sec not in MOVIEGRAPH_VALID_FRAMES:
                continue
...
def parse_bboxes_file(
    ...
    """
    Parse MOVIEGRAPH bounding boxes files.
    Args:
        ann_filenames (list of str(s)): a list of MOVIEGRAPH bounding boxes annotation files.
        ...
        with PathManager.open(filename, "r") as f:
            for line in f:
                ...
                if video_name not in all_boxes:
                    ...
                    for sec in MOVIEGRAPH_VALID_FRAMES:
